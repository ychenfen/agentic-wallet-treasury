...
const SOURCE_PATH = resolve(ROOT, "contracts/src/AgenticTreasury.sol");
const ARTIFACT_PATH = resolve(ROOT, "contracts/out/AgenticTreasury.sol/AgenticTreasury.json");
...
async function main(): Promise<void> {
  ...
  const contract = output.contracts?.["AgenticTreasury.sol"]?.AgenticTreasury;
  if (!contract) throw new Error("AgenticTreasury contract output missing.");
