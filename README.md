...
| `apps/web/public/byreal-probe.json` | `npm run byreal-probe` | Real Byreal RealClaw CLI capability + pool probe. |
| `apps/web/public/contract-verification.json` | `npm run prepare-verification` | Mantlescan verification package summary. |
...
| 1 — Byreal probe | `npm run byreal-probe` | Captures Byreal CLI version, catalog, DEX overview, pools, and analysis. |
| 2 — Deploy | `npm run deploy-treasury` | Builds + deploys `AgenticTreasury` to Mantle Sepolia. |
| 2 — Verify prep | `npm run prepare-verification` | Writes Mantlescan Standard JSON inputs + constructor args. |
...
| 维度 | 结论 |
|---|---|
| 截止日期 | 2026-06-15 23:59，剩 14 天（截至 2026-06-01） |
...
2. `PROJECT_STATE_AND_ROADMAP.md` —— 当前全局状态和后续完整规划
3. `hackathon/01-requirements-criteria.md` —— 官方评分标准和部署奖要求
4. `strategy/05-winning-plan.md` —— 当前执行方案
5. `strategy/04-strategy-and-next-steps.md` —— 原始赛道分析
6. `tracks/T6-agentic-wallets.md` —— 赛道 6 深入
7. `erc-8004/03-erc8004.md` + `byreal/02-byreal-realclaw.md` —— 实操前再读一遍
8. `mantle/01-mantle-stack.md` —— 写代码时随时回查
